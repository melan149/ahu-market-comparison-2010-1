...
if any(s['brand'] == "(any)" for s in selections):
    ...
    def render_data_row(cols, col_name, df_list, num_units, colors):
        ...
        for i in range(num_units):
            with row_cols[i+1]:
                df_comp = df_list[i]
                # Use the column name directly in the markdown display if it's not a known column
                display_col_name = col_name if col_name in df_comp.columns else col_name 

                if not df_comp.empty and display_col_name in df_comp.columns:
                    val = df_comp[display_col_name].iloc[0]
    ...
    sections_config = [
        ...
        # --- RESTRUCTURED SECTION: OVERALL DIMENSIONS ---
        {
        ...
        # --- END RESTRUCTURED SECTION: OVERALL DIMENSIONS ---
        {
        ...
        # --- UPDATED SECTION: SILENCER DATA (Chart removed) ---
        {
            "title": "Silencer data",
            "rows": [
                silencer_casing_col,
            ...
            "charts": [] # Chart moved to Overall dimensions
        },
        ...
        with st.expander(f"Show {section['title']} details", expanded=False): 
            ...
            for chart_name in section["charts"]:
                    ...
                    for i, s in enumerate(selections):
                        ...
                        if brand and brand != "(any)" and unit and recovery:
                            temp_df = df_competitor[
                                ...
                                (df_competitor[col_comp_quarter] == s.get('quarter')) & # <-- FIXED SYNTAX ERROR HERE
                ...
                # --- CHART 2: Internal Cross Section Area (Supply Fan) Shape ---
                    ...
                    # This chart is nested under Overall dimensions (MOVED)
                    fig = go.Figure()
                    all_x, all_y = [], []
