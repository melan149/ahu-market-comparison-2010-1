        ...
        with st.expander(f"Comparison {i+1}"):
            ...
            if selected_brand != "(any)":
                ...
                if selected_recovery == "RRG" and col_comp_type and not df_competitor_selection.empty:
                    ...
                    # Ensure selection is consistent with the single row found
...
if any(s['brand'] == "(any)" for s in selections):
    ...
    for col_name in df_competitor.columns:
        ...
        if col_name == internal_height_supply_filter_col: 
            ...
            # 2. CRITICAL: Unit Cross Section Area vs Unit Size (Redesigned)
    ...
    for item in all_cols_ordered:
        if item["type"] == "header":
            ...
            # --- REDESIGNED CHART: Unit Cross Section Area (Supply Filter) vs Unit Size (Scatter) ---
            elif chart_name == "chart_area_vs_size":
                full_chart_data = []
                unique_y_labels = [] # To control the Y-axis order
                ...
                for i, s in enumerate(selections):
                    ...
                    # Only proceed if the necessary filters are selected
                    if brand and brand != "(any)" and unit and recovery:
                        # Filter the master competitor DataFrame based on all selected non-size/type/material filters
                        ...
                        # Apply Type/Material filter if applicable, matching the single-unit selection logic
                        ...
                        # Get data points for ALL available Unit Sizes in this filtered subset
                        if not temp_df.empty and unit_cross_section_area_supply_filter_area_col in temp_df.columns and col_comp_size in temp_df.columns:
                            ...
                            # Filter for uniqueness based on size and area (in case a size is listed multiple times)
                            ...
                            for _, row in unique_sizes_df.iterrows():
                                ...
                                # Add point if area is a valid, positive number
                ...
                if full_chart_data:
                    ...
                    # Ensure Y-axis is ordered correctly (Plotly sorts categories alphabetically by default)
                    # Reverse the list for a top-down natural reading order based on insertion
                    ...
                    # Apply custom category ordering for Y-axis
                    fig.update_layout(
                        yaxis={
                            'categoryorder': 'array', 
                            'categoryarray': unique_y_labels
                        },
                        # Ensure X axis starts at 0 for a fair comparison
                        xaxis=dict(range=[0, chart_df['Area (m²)'].max() * 1.15], title='Area (m²)'),
                        height=200 + 30 * len(unique_y_labels) # dynamic height adjustment
                    )
                    st.plotly_chart(fig, use_container_width=True)
                else:
