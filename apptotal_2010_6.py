...
# NEW MAPPING for the requested Area vs Size chart
...
if any(s['brand'] == "(any)" for s in selections):
    ...
    for col_name in df_competitor.columns:
        ...
        if col_name == internal_height_supply_filter_col: 
            ...
            # 2. NEW: Area vs. Size scatter plot
    ...
    # Add the newly mapped column to the exclusion list so it only appears in the chart
        ...
        if item["type"] == "header":
            ...
            if chart_name == "chart1":
                ...
                if fig.data:
                    ...
                    # Enforce 1:1 scale and ensure axes start at 0
                    fig.update_yaxes(scaleanchor="x", scaleratio=1)
                    fig.update_layout(
                        title='Internal Cross Section Area (Supply Filter) [mm]', 
                        xaxis_title='Width (mm)', 
                        yaxis_title='Height (mm)',
                        xaxis=dict(range=[0, max_x * 1.05]), # Starts at 0
                        yaxis=dict(range=[0, max_y * 1.05]), # Starts at 0
                        hovermode="closest"
                    )
                    st.plotly_chart(fig, use_container_width=True)

            # NEW CHART: Unit Cross Section Area (Supply Filter) vs Unit Size
                ...
                for i, df_unit in enumerate(filtered_dfs_competitor):
                    if not df_unit.empty and unit_cross_section_area_supply_filter_area_col in df_unit.columns:
                        area_val = df_unit[unit_cross_section_area_supply_filter_area_col].iloc[0]
                ...
                if chart_data:
                    ...
                    fig.update_yaxes(range=[0, chart_df['Area (m²)'].max() * 1.05])
                    st.plotly_chart(fig, use_container_width=True)
                ...
                for i, df_unit in enumerate(filtered_dfs_competitor):
                    if not df_unit.empty:
                        ...
                        if is_circ:
                            ...
                            # Shifted center to place the bottom-left of the bounding box at (0, 0)
                            x_center = radius
                            y_center = radius
                            
                            # Update max bounds for the axis starting at 0
                            ...
                            if x_vals and y_vals:
                                ...
                                # Now uses 'lines+markers' for consistency
                ...
                for i, df_unit in enumerate(filtered_dfs_competitor):
                    if not df_unit.empty and all(c in df_unit and pd.notna(df_unit[c].iloc[0]) for c in [capacity_range1_col, capacity_range2_col, capacity_range3_col]):
                        label = f"Unit {i+1}: {selections[i]['brand']} - {selections[i]['size']}"
                        chart_data.append({"Capacity Range": "Range 1", "Value (kW)": df_unit[capacity_range1_col].iloc[0], "Selection": label})
                        chart_data.append({"Capacity Range": "Range 2", "Value (kW)": df_unit[capacity_range2_col].iloc[0], "Selection": label})
                        chart_data.append({"Capacity Range": "Range 3", "Value (kW)": df_unit[capacity_range3_col].iloc[0], "Selection": label})
                if chart_data:
                    chart_df = pd.DataFrame(chart_data)
                    fig = px.bar(chart_df, x="Capacity Range", y="Value (kW)", color="Selection", barmode="group", title='Electrical Heater Capacity (kW)')
                    # Set Y axis to start at 0
                    fig.update_yaxes(range=[0, chart_df['Value (kW)'].max() * 1.05])
                    st.plotly_chart(fig, use_container_width=True)
