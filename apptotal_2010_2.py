...
import plotly.graph_objects as go # <--- FIX: Added the import for 'go'
import numpy as np
...
def load_competitor_data():
    try:
        # Assuming Data_2025_2.xlsx is in the same directory
        return pd.read_excel("Data_2025_2.xlsx", sheet_name="data", engine='openypxl')
    except FileNotFoundError:
    ...
    for item in all_cols_ordered:
        if item["type"] == "header":
            ...
            if chart_name == "chart1":
                ...
                # The fix is here: 'go' is now imported as 'plotly.graph_objects'
                fig = go.Figure()
                for i, df_unit in enumerate(filtered_dfs_competitor):
                    if not df_unit.empty:
